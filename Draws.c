...
void DrawScreen(void )
{
    ...
    switch(MainStatusMachine)
	   ...
	   case MSM_GET_PAR:
	       ClearPage(0);
	       DisplaySString ("Get parameters",3,1,0);
	   break;
   /*************************************************************/
   /*************************************************************/
   /*************************************************************/
   /*************************************************************/
	   ...
	   case MSM_SET0:
	       ClearPage(0);
		   DisplaySString("Sweep parameters:", 15, 0,0);

		    DisplaySString("DTStart", 0, 1, selected_Parameter == SELECTED_TStart ? 1 : 0);
		    DisplaySString("1Strk", 64-15 , 1, selected_Parameter == SELECTED_T1st ? 1 : 0);
		    DisplaySString("DTStop",128-38, 1, selected_Parameter == SELECTED_TStop ? 1 : 0);
   ...
   /*************************************************************/
	   ...
	   case MSM_SET1:			 
	       ClearPage(0);
		   DisplaySString("Sweep duration:", 15, 0,0);

		    DisplaySString("DlyON", 0, 1, selected_Parameter == SELECTED_OnDelay ? 1 : 0);
		    DisplaySString("Duration", 64-24, 1, selected_Parameter == SELECTED_SWEEP ? 1 : 0);
		    DisplaySString("DlyOff", 128-38, 1, selected_Parameter == SELECTED_OffDelay ? 1 : 0);
	   ...
	   case MSM_SET2:
	       ClearPage(0);
		   DisplaySString("Feedback:", 15, 0,0);

		    DisplaySString("Kr", 0, 1, selected_Parameter == SELECTED_Kr ? 1 : 0);
		    DisplaySString("Ki", 64-6, 1, selected_Parameter == SELECTED_Ki ? 1 : 0);
		    DisplaySString("Kd", 128-16, 1, selected_Parameter == SELECTED_Kd ? 1 : 0);
	   ...
	   case MSM_SET3:			  // Display & set   Amplify, TriggerLevel, BlindPeriod
	       ClearPage(0);
 		   Chenar();
		   DisplaySString("Trigger detect:", 10, 0,0);
		   ...
		   DisplaySString("Amp", 2, 1, selected_Parameter == SELECTED_Amplify ? 1 : 0);
		   DisplayS3Int(Amplify, 2, 2, selected_Parameter == SELECTED_Amplify ? 1 : 0);
		   DisplaySString("Trg", 2, 4, selected_Parameter == SELECTED_Level ? 1 : 0);
		   DisplayS3Int(TriggerLevel, 2, 5, selected_Parameter == SELECTED_Level ? 1 : 0);
		   DisplaySString("Blind", 88, 1, selected_Parameter == SELECTED_BlindPeriod ? 1 : 0);
		   DisplayS3Int(BlindPeriod, 88, 2, selected_Parameter == SELECTED_BlindPeriod ? 1 : 0);
		   DisplaySString("Clr", 64, 4, selected_Parameter == SELECTED_BlindLevel ? 1 : 0);
...
*/
	   ...
	   case MSM_SLIDE:
	       ClearPage(0);
 		   ...
 		   DisplaySString("Slide mode - manual:", 5, 0,0);
		   DisplaySString("SV1",7, 1,0);
		   DisplaySString("SV2",55,1,0);
		   DisplaySString("SV3",55,6,0);
		   DisplaySString("SV4",103,1,0);
	   ...
	   case MSM_SWEEP:
 	       ClearPage(0);
		   DisplaySString("Sweep mode:", 15, 0,0);
		   Chenar();
		   if(KeyPlusTime<-5) 
			   {
	       SelUART1SendTxBuffer(">MACC\r");
		   ...
		   switch(WMessage[2])
		        {
				case '3': case '1':
 			      DisplaySString("Discharging", 5, 4,0);
			      DisplaySString(&WMessage[4],0,7,1);
			      WMessage[2]=0;
				break;
				default:
		           ; //DisplaySString("Press ^ to discharge",3, 5,0);
			    }

		    DisplaySString("Waiting for START",10, 3,0);
	   ...
	   case MSM_RUN:
		   ...
		   switch(WMessage[2])  {
			  case '0':	// Delay ON
		          DisplaySString("Delay ON", 40, 4,0);
				  DisplaySString(&WMessage[4],0,7,1);
			  break;
			  case '1':	 // Sweep
		       DisplaySString("Sweep ON", 40, 0,0);
				  //DisplaySString(&WMessage[4],0,7,1);
				  // W1nnnnxxxx
			  ...
			  break;
			  case '2':	 // Delay Off
		          DisplaySString("Delay OFF", 40, 4,0);
				  DisplaySString(&WMessage[4],0,7,1);
			  break;
              };
		WMessage[0]=0;	  		
	   ...
	   case MSM_STOP_SWEEP:
 	       ClearPage(0);
		   DisplaySString("Stop sweep:", 15, 0,0);
		   Chenar();
			switch(LastKey)
			  {
			  case KEY_KBSET:    LastCommand = LC_SET_MODE ; break;
	   ...
	   case MSM_ERR_COM:
	       ClearPage(0);
	       DisplaySStringD ("ERR COM",15,1,0);
	       DisplaySString ("Retrying...",3,5,0);
	   break;
		case MSM_GO_SET0:
		case MSM_GO_SLIDE:
		case MSM_GO_SWEEP:
	   ...
	   default:   
 	       ClearPage(0);
		   DisplaySString("Unknown mode:", 15, 0,0);
		   DisplaySString("Press one mode key!",3,4,0);
		   Chenar();
			switch(LastKey)
			  {
			  case KEY_KBSET:     LastCommand = LC_SET_MODE ; break;
