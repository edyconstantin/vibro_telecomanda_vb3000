// Message.c - protocolul cu vibsist (port pe ATmega64A, fara stdio)
#include "globals.h"
#include "fmt.h"
#include "uart0.h"
#include "Message.h"
...
// helper: ">XX,n1[,n2[,n3[,n4]]]\r" fara stdio
static void msg_build(char *dst, char c1, char c2, int nvals, int *vals)
{
    unsigned i = 0, k;
    dst[i++] = '>';
    dst[i++] = c1;
    dst[i++] = c2;
    for (k = 0; k < (unsigned)nvals; k++) {
        dst[i++] = ',';
        i += fmt_num(dst + i, vals[k], 3);
    }
    dst[i++] = '\r';
    dst[i] = 0;
}
...
{
    char text[24];
    int v[3] = {Tstart, Tstop, F1Strike};
    msg_build(text, 'S', 'H', 3, v);
...
{
    char text[24];
    int v[3] = {TSweep, TDelayOn, TDelayOff};
    msg_build(text, 'S', 'W', 3, v);
    SelUART1SendTxBuffer((unsigned char *)text);
}
...
{
    char text[24];
    int v[3] = {KProportional, KIntegral, KDiferential};
    msg_build(text, 'S', 'K', 3, v);
    SelUART1SendTxBuffer((unsigned char *)text);
}
...
{
    char text[28];
    int v[4] = {Amplify, TriggerLevel, BlindPeriod, BlindLevel};
    msg_build(text, 'S', 'D', 4, v);
    SelUART1SendTxBuffer((unsigned char *)text);
}
            ...
            switch (msg[1]) {
                    ...
                    switch (msg[2]) {
                        ...
                        case 'C': // get last command
                            if (LastCommand & LC_H_MODIFIED) SendHBuff(), LastCommand ^= LC_H_MODIFIED;
                                ...
                                if (LastCommand & LC_SET_MODE)   SelUART1SendTxBufferP(">MS\r");
                                if (LastCommand & LC_SLIDE_MODE) SelUART1SendTxBufferP(">ML\r");
                                if (LastCommand & LC_SWEEP_MODE) SelUART1SendTxBufferP(">MW\r");
                                if (LastCommand & LC_RUN_START)  SelUART1SendTxBufferP(">MR\r");
                                if (LastCommand & LC_RUN_STOP)   SelUART1SendTxBufferP(">MZ\r");
                                if (LastCommand & LC_SLIDE_ACT)  SelUART1SendTxBuffer(VText);
                                LastCommand = LC_NONE;
                            }
                            break;
                    }
                ...
                case 'F': // mesaje "FORCE"
                    switch (msg[2]) {
                        ...
                        case 'S': case 's': MainStatusMachine = MSM_GO_SET0; break;
...
// BUG corectat fata de original (vmax/vmin inversat). Nu e apelata in cod
// (cod mort), dar e corecta acum daca o s-o folositi.
